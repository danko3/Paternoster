...
;   The commumication with the database runs via a Perlscript paternoster.pl.
;   This script lives on juw15/export/jive/jops/bin/tapes/user/.
...
;   PN[version]2.ahk; putty.exe; private2.ppk; TrackSend.ahk; SerialPort.ahk --> path:
...
Input:
  ...
  If Len = 0
    {
      ...
      FileAppend, [%timestamp%]  Input stringlength '%CtrlContents%' was 0`n, %logfile%
      CtrlContents = %VSN%   ;Put it in the Input.
    }
  ...
  If Action = Send
    {
      command = -Send -v %CtrlContents%    ;Remove vsn from Paternoster, set vsnStatus = sent, Existence = History
      ...
      FileAppend, [%timestamp%]`n, %VSNSend%  ; Write a timestamp in the file VSNSend when starting.
      FileAppend, %CtrlContents%`n, %VSNSend%  ; Just put the VSN in a file for Track.
      ...
      SoundSet, +50
      SoundPlay, c:\windows\media\Blip.wav  ; ting!
  ...
  Else
    {
      ...
      If Action <> Confirm
        ;  MsgBox, new vsn %CtrlContents%
          Position := Find(CtrlContents)   ; Goto subroutine 'Find'
  ...
  Else If Action = Put
    {
      ...
      Position := Put(CtrlContents)    ; Now Goto 'Put'. redefine Position
...
Find(VSN_Asked)
  {
    ...
    IfNotInString, retText, Error
    {
        ...
        If action = Get
          {
            Answer := Paternoster(CurrentCarier,Station)   ; Position the paternoster.
...
Put(VSN_Asked)
  {
    ...
    Loop, %Output0% ; (Output0 is the Number of elements)
      {
            ...
            IfInString, Semaphore, Ready
              {
                ...
                ;break   ; stop after the first free position.
              }
            ;Else
            ;    SoundPlay, Low.wav, wait
...
GetIt:
  ...
  While not success
  {
    Loop
      {
        ...
        StringGetPos, pos, retText, paternosterUpdate.pl -%Command%,R1 ; reverse search for last command
        StringTrimLeft, retText, retText, %pos%  ; remove all text up to the last command
        StringReplace retText, retText, paternosterUpdate.pl -%Command% ; remove last command itself
        ...
        IfInString, retText, )?
          {  ;needs to be tested (25-09-2012)
            ControlSend,,y{Enter}, %RemoteMachine%  ;on eg. CORRECT>paternosterUpdate.pl -e em081a (y|n|e|a)?
    ...
    FileAppend, GetIt took %ElapsedTimeGetIt% ms for command:{%command%}`n, %logfile%
    Return
  }
Return
...
ClosePN:
  ...
  IniWrite, %CurrentCarier%, PN.ini, CurrentCarier, Carier
...
InputCheck:
  ...
  If Len = 0
    {
      MsgBox,,InputCheck, VSN asked is %CtrlContents%, length = %Len%`n(VSN= %VSN% Action = %Action%) ; debug line
      If Send
        {
          GuiControl,, Button11, 0 ; Switch Checkbox off
          Gosub, CheckboxS
        }
      Else If Receive
        {
          GuiControl,, Button9, 0 ; Switch Checkbox off
          Gosub, CheckboxC
        }
      Else If Removed
        {
          GuiControl,, Button10, 0 ; Switch Checkbox off
          Gosub, CheckboxR
        }
      Gosub, ButtonCancel
      GuiControl,, Input  ; rewrite the Input field. (Empty it.)
