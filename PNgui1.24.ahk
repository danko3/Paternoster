...
;SetControlDelay, 150
Process, Priority, , High   ; Get in front of that #@$% virusscanner
OnMessage(0x53, "WM_HELP")  ; needed for the help button in msgbox.
...
IniRead, Positions, PN.ini, PaternosterStrings, Positions
IniRead, Putty, PN.ini, Files, Putty
...
IniRead, Carier, PN.ini, Carier, Carier 
IniRead, IdleTime, PN.ini, Timer, IdleTime 
IniRead, Station, PN.ini, StartStation, Station
...
IniRead, SetPath, PN.ini, Remote, SetPath
...
;ControlSend,, alias PN paternosterUpdate.pl{Enter}, %RemoteMachine%
...
Gui:
  ...
  Gui, Add, Edit, x70 y535 w125 vAmount limit1  ;Edit1
  Gui, Add, Button, x20 y533 vok, OK
  ;Gui, Add, Button, x180 y533 End, End
  ;Gui, Add, Text, x250 y185, VSN
  Gui, Add, GroupBox, x242 y187 w425 h130 vVSNbox, VSN
  Gui, Font, S60 CDefault, Courier New Bold
  ;Gui, Font, S50 CDefault, Lucida console Bold
  Gui, Add, Edit, x250 y207 w410 Uppercase Limit18 vInput  ;Edit2
  Gui, Font, S15 CDefault, Verdana
  Gui, Add, Button, x250 y15 w105 h50 vProject, Project
  Gui, Add, Button, x365 y15 w70 h50 vGet, &Get
  Gui, Add, Button, x445 y15 w70 h50 vPut, &Put
  Gui, Add, Button, x525 y15 w105 h50 vCancel, &Cancel  ; button6
  ;Gui, Font, S10 CDefault, Verdana
  Gui, Add, GroupBox, x693 y117 w124 h46 border vexpedition
  Gui, Add, Button, x695 y120 w120 h40 vSouterain, Souterain
  Gui, Font, S10 CDefault, Verdana
  Gui, Add, GroupBox, x663 y13 w157 h80 vShippingbox, Shipping
  ...
  Gui, Add, Button, x770 y35 w45 h40 gEnd, End  ; button 13
  ...
  Gui, Add, Edit, x740 y410 w50 limit2 number left vCarier ;Edit3
  Gui, Font, S12 CDefault, Verdana
  Gui, Add, Button, x740 y360 w75 h40vGo, Go
 ; Gui, Add, Button, x670 y440 w65 h40, Dummy
  Gui, Add, Button, x740 y460 w75 h40 vInit, Init  ;button 20
  Gui, Font, S10 CDefault, Verdana
  Gui, Add, GroupBox, x362 y542 w305 h60, System Messages  ;Button21
  Gui, Font, S15 C4444ff, Verdana
  Gui, Add, Edit, x370 y560 w240 vSystem limit1, %Semaphore%   ; System messages 
  Gui, Add, Edit, x620 y560 w40 vReturnedCarier limit1, %ReturnedCarier%    ; Carier Returned from PN. (Edit5)
  Gui, Font, s12, MS sans serif
  Gui, Add, StatusBar,,
  Gui, Add, Progress, vMyProgress w100 h20 x570 y610 Range0-%IdleTime% ;-Smooth
  Gui, Add, Button, Default Hidden, Enter
  Gui, Show, w840 h630, PN v. %PNVersion%
...
anchor("")
  ...
  SetTimer, Tip, 2000
  SB_SetParts( 180, 150, 70, 70, 100, 100) ; divide the statusbar in 6 parts.
...
;SB_SetProgress, %ITime%, 5  ; Needs extra library
...
GuiSize:
  ...
  Anchor("Messages", "y") ; GroupBox system messages
  Anchor("Amount", "y")                 ; Amount of disks in project
  Anchor("System", "y")                ; System messages
  Anchor("ReturnedCarier", "y", true)           ; returned carier
...
Tip:
  ...
  If ACtrl = Edit3
      Tip = Wanted shelf `nPress Init to edit
  ...
  If ACtrl = msctls_StatusBar321
    {
      MouseGetPos, XCtrl
      If XCtrl Between 5 and 180     ; part 1
          Tip = Read VSN
      If XCtrl Between 185 and 330   ; part 2
          Tip = Checked VSN
      If XCtrl Between 335 and 400   ; part 3
          Tip = Position
      If XCtrl Between 405 and 472   ; part 4
          Tip = Putty.log size
       If XCtrl Between 472 and 570  ; part 5
          Tip = Free places in Paternoster
      If XCtrl <> %LastXCtrl%
        {
          ToolTip, %Tip%
          LastXCtrl = %XCtrl%
...
HelpAbout:
  ...
  Gui, 2:Add, Text,, PN  Version %PNVersion%`n`nDate: March 2012`n
  ,AutoHotkey Version: %A_AhkVersion% `nAuthor: H. Tenkink
...
ButtonProject:
  ...
  GuiControl, Disable, vInput  ;Edit2
...
ButtonCancel:
  ...
  GuiControl, Enable, vInput  ;Edit2
  ...
  GuiControl, Enable, System ; Groupbox System messages
      ...
      IfExist, %VSNReceive%
        {
          ...
          RunWait http://magnolia.nrao.edu/track/
          WinWait, Connect to magnolia,,5  ; Wait for inlog box
          Send, {Enter}
          WinWait, VLBA Tape TRACKING,,5 ; Wait for TRACK window
          MouseClick,, 110, 345 ; Hit receieve button.
        }
      if !WinActive(TRACK)
      {
              ...
              If not Errorlevel
                {
                 ...
                 ; Run, TrackSend.ahk
                 RunWait http://magnolia.nrao.edu/track/
                 WinWait, Connect to magnolia,,5  
                 Send, {Enter}
                }
              if !WinActive(TRACK)
              {
              Gosub, ButtonCancel
              AmountSent = 0 ; reset the Sent counter
              }
            }
Return
      ...
      IfExist, %VSNShip%
        {
          ...
          Clipboard = %Send%  ;Copy the VSn's to the clipboard
          RunWait, TrackSend.ahk
        }
      Gosub, ButtonCancel
    }
Return
...
Carier--  ; subtract 1 from CurrentCarier.
If Carier = 0
  Carier = 32
GuiControl, Focus, Input
Return
...
ButtonGo:
  GuiControlGet, Carier,,Carier  ;Get the Input.
  If ErrorLevel
      MsgBox,16,, Go Error: Carier: %Carier% not found.
  StringLen, len, Carier
  If (len = 0)
      MsgBox,48,, Go Carier not given.%Carier%
  Else
  ...
  Else
    {
      ...
      InitString = %Init%%HCarier%%Fill%%NrCariers%%Stat%%Positions%%Fill21%%End%
...
If FileExist("putty.log")
 ...
 ; Run, %start%,,, %RemoteMachine%   ; start putty (debug)
      ...
      If not Errorlevel  ; if we see a prompt
        {
          ...
          If ErrorLevel = 0
              MsgBox, 16, Error 2, prompt: %CommandPrompt% not found `n %retText%.
          Break
        }
      If A_Index > 30
        {
...
GuiControl, Disable, vAmount  ;Edit1
GuiControl, Disable, OK
GuiControl, Disable, Project
GuiControl, Disable, Get
GuiControl, Disable, Put
...
if Disable_All
{
GuiControl, Enable, vHistory  ;ListBox1
GuiControl, Enable, vAmount  ;Edit1
GuiControl, Enable, vInput  ;Edit2
GuiControl, Enable, OK
GuiControl, Enable, VSN
GuiControl, Enable, Project
